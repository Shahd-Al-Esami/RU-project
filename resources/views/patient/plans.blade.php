...
<body>
<a href="{{ url('home') }}" class="btn btn-secondary go-back-btn">Go Back</a>
    ...
    <table>
        ...
        <tbody>
            @foreach ($planOrders as $order)
                <tr>
                    <td>{{ $order->id }}</td>
                    <td>{{ App\Models\User::where('id', $order->doctor_id)->firstOrFail()->name ?? 'N/A' }}</td>
                    <td>{{ $order->goals }}</td>
                    ...
                    <td>
                        ...
                        <a href="{{ route('getPlan',['plan_order_id'=>$order->id]) }}" class="btn follow-btn">Display the Plan</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
