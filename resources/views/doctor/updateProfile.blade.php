...
<a href="{{ url('myProfile') }}" class="btn btn-secondary go-back-btn mt-4 mx-4">Go Back</a>

<div class="container">
    <!-- Styled and Decorated Create/Edit doctor Section -->
    <form action="{{ route('updateProfile') }}" method="POST" enctype="multipart/form-data" style="position: relative;">
        @csrf
        <h3>Personal Information</h3>
        <label for="name">Name:</label>
        <input type="text" id="name" name="name" required value="{{ old('name', $doctor->name ?? '') }}">

        <label for="email">Email:</label>
        <input type="email" id="email" name="email" required value="{{ old('email', $doctor->email ?? '') }}">

        <label for="password">Password:</label>
        <input type="password" id="password" name="password" required placeholder="Enter new password">

        <label for="country">Country:</label>
        <input type="text" id="country" name="country" required value="{{ old('country', $doctor->country ?? '') }}">

        <label for="age">Age:</label>
        <input type="number" id="age" name="age" required value="{{ old('age', $doctor->age ?? '') }}">
        ...
        <input type="text" id="phone_number" name="phone_number" required value="{{ old('phone_number', $doctor->phone_number ?? '') }}">
...
@if(!auth()->user()->role ==='admin')
        ...
        <textarea id="bio" name="bio" required>{{ old('bio', $doctor->doctorInformation->bio ?? '') }}</textarea>


        <h3>Doctor Holiday</h3>
        <label for="day">Day:</label>

        @php
            $daysOfWeek = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
            $selectedDays = $doctor->doctorHoliday->pluck('day')->toArray(); // Existing selected values
        @endphp

        <select id="day" name="day[]" class="form-control" multiple required>
            @foreach ($daysOfWeek as $day)
                <option value="{{ $day }}" {{ in_array($day, old('day', $selectedDays)) ? 'selected' : '' }}>
                    {{ $day }}
                </option>
            @endforeach
        </select>
